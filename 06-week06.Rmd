...
<center>*Available Wednesday morning*</center>
...
We'll be using a **single data download from the London Datastore**, which we'll need to clean, and then investigate different factors that might contribute to obesity in children.
...
This report is pretty old - but this gives us opportunity to investigate into the current(-ish) situation of childhood obesity and see if these findings still were similar 10 years later.
...
The thing with spatial distributions is that we can quickly pick up on spatial patterns present within our data (although also miss others!) -but considering we're just presenting our data on a map, how can we account and even quantify these clusters?
...
If we want to come up with quantifiable descriptions of a variable's distribution and how they vary over space, then we need to find ways of quantifying these clusters. 
...
There can appear to be clusters, or the distribution can be random. 

If you want to explain and discuss variables, the underlying causes, and the possible solutions to issues, it becomes useful to quantify how clustered, how random, or how dispersed, these distributions are. 
...
We can assess the distribution of our data using spatial autocorrelation tests, which can be conducted on both a **global** (identify if the data is clustered) and **local** (identify the precise clusters) scales. 

There are also various spatial autocorrelation tecnhiques that can be used - however they all require one piece of information: **how we define 'neighbours' in our dataset to enable the value comparison.**
...
library(vembedr)
embed_msstream('0') %>% use_align('center')
```
...
If you want to calculate the relative spatial relation of distributions, knowledge of what counts as a “neighbour” becomes useful. 

Every observation in a dataset will have a set of neighbours - these are the observations that surround our initial observation within a specific **proximity**.

This proximity or closeness can be defined in a multitude of ways, including:

* **Contiguity**: Queen / Rook (*plus order*)
* **Fixed Distance**: Euclidean Distance (e.g. within 2km)
...
As shown in the lecture, we have several types of tests that look to quantify spatial autocorrelation. Of these tests, there are two categories:

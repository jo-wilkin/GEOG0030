...
london_stations_osm <- london_stations_osm$osm_points %>% st_set_crs(4326) %>%
  # Reproject our dataset to BNG for future use
  st_transform(27700) %>%  
  # And clip our osm data to our london outline shapefile  
  st_intersection(london_outline) %>% select(c("osm_id", "name", "network", "operator", "public_transport", "railway"))
```
